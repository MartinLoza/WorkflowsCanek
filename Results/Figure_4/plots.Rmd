...
## Functions
## Plot for BioRxiv v1
plotCorrection <- function(object = NULL, title = "correction"){
  p1 <- DimPlot(object, reduction = "pca", group.by = "batch") + 
    ggtitle(title) + mytheme
  p2 <- DimPlot(object, reduction = "umap", group.by = "batch") + mytheme + theme(plot.title = element_blank())
  p3 <- DimPlot(object, reduction = "pca", group.by = "celltype") + mytheme + theme(plot.title = element_blank())
  p4 <- DimPlot(object, reduction = "umap", group.by = "celltype") + mytheme + theme(plot.title = element_blank())
...
```{r fig.height=55, fig.width=20}
plots <- lapply(names(datal), function(n){
  if(n == "GS")
    title = "Gold Standard"
  else
    title = n
  return(plotCorrection(object = datal[[n]], title = title))
})
